...
本实验的总成绩由**代码**和**报告**两部分组成，各占一半。只要认真做了，就能拿到不错的成绩。
...
**通过全部基础层测试就是 75 分。** 基础层覆盖的都是终端复用器的核心功能，把这些做对就已经很好了。进阶和挑战是额外加分项，做到多少算多少。

每个层级内的测试用例等权。层级得分 = 该层通过率 x 层级满分。

#### 报告评分（50%）

报告或 Presentation（见第 5 节），重点看你的学习过程和 AI 协作质量，不要求代码完美。具体评分标准后续通知。

**额外加分**：选择 Presentation 的同学可以获得额外加分，加分可以补到代码部分的得分上。

#### 盲测

学期内进行两次（中期检查和最终提交各一次），服务端自动运行全部测试用例，返回每个层级和类别的通过状态与得分。不返回具体哪个测试用例失败或失败原因。盲测使用与 Public 同分布但更多实例、更长运行时间的测试用例，不引入任何新的负载类型或测试维度

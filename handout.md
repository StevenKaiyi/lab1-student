...
测试用例按功能难度分为三个层级，满分 100 分：

| 层级 | 分值 | 覆盖功能 | 说明 |
|:-----|-----:|:---------|:-----|
| 基础 | 60 | Pane 创建/销毁/切换、基础 IO、信号隔离、僵尸回收、窗口 resize、压力测试、Server 生命周期 | 通过全部基础测试即得 60 分。这些是终端复用器的核心功能，也是系统编程的基本功 |
| 进阶 | 25 | Detach/Reattach、Log、Pipeout | 需要理解 session 管理、fd 生命周期和进程间协作 |
| 挑战 | 15 | 多 Client 同时连接、Layout 分屏、Capture 屏幕捕获 | 并发共享状态，难度最高 |

每个层级内的测试用例等权。层级得分 = 该层通过率 x 层级满分。例如基础层 19 个测试通过 16 个，得分为 16/19 x 60 ≈ 50.5 分。

**建议按层级顺序实现**：先拿稳基础 60 分，再逐步攻克进阶和挑战。

**盲测**：学期内进行两次（中期检查和最终提交各一次），服务端自动运行全部测试用例，返回每个层级和类别的通过状态与得分。不返回具体哪个测试用例失败或失败原因。盲测使用与 Public 同分布但更多实例、更长运行时间的测试用例，不引入任何新的负载类型或测试维度
...
## 5 报告或 Presentation（二选一）

你需要从以下两种形式中选择一种提交，核心内容相同：不是展示 mini-tmux 本身，而是你在整个过程中如何学习、如何与 AI 协作。

### 选项 A：书面报告

提交一份 PDF 报告，放在仓库的 `report/` 目录下。篇幅不限，但请做到言之有物。

### 选项 B：Presentation

录制或现场演示均可，具体安排我们后续通知。

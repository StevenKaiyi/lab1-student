...
Presentation 不是功能演示，也不是让你背诵 man page。我们想听你讲故事：你在实现过程中遇到了什么问题，你是怎么定位的，你最终理解了什么。

一个只实现了单 Pane 但能清晰解释 PTY、进程组、信号投递原理的同学，评价会高于一个实现了所有功能但说不清 `setsid()` 为什么必要的同学。
...
11. 描述一下一个按键从用户敲下键盘到最终出现在 Pane 程序中的完整数据路径（提示：用 `strace -e read,write -p <pid>` 分别跟踪 Client 和 Server 进程，观察数据流向）

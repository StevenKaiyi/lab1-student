...
Server 与 Client 通过 Unix domain socket 通信。你需要自己设计 Server 与 Client 之间的通信协议，这是一个重要的设计决策：Server 是直接转发各 PTY 的原始字节流让 Client 自己渲染，还是在 Server 端完成渲染后推送结构化的屏幕数据？两种方案各有优劣，请自行权衡。
...
在非只读 Client 中，通过前缀键进入命令模式并输入命令。命令以冒号开头：
...
这不是一个独立的功能模块，而是贯穿整个实现的要求。Server 在运行过程中会创建大量子进程（Pane shell、pipeout 命令等）和 fd（PTY、socket、pipe、日志文件等），必须严格管理它们的生命周期。
...
项目仓库中的 `workloads/public/` 目录包含所有公开测试用例的 YAML 描述文件。你可以直接阅读这些文件来了解每个测试用例在测什么、执行了哪些步骤。
...
你可以阅读 YAML 文件来精确了解每个测试的步骤和验证点，但不能在本地运行自动评测。评测只能通过提交后在服务端运行。
...
- `make grade` 只返回总分（格式：`Score: X/Y`），不返回每个用例的详细结果
- **Private 盲测**：学期内进行两次（中期检查和最终提交各一次），使用与 Public 同分布但更多实例、更长运行时间的测试用例，只返回总分。Private 不引入任何新的负载类型或测试维度
...
make grade      # 通过 Docker 运行公开测试用例并评分
```
...
Presentation 不是功能演示。我们更关心你对系统概念的理解深度，而非你的 mini-tmux 实现了多少功能。一个只实现了单 Pane 但能清晰解释 PTY、进程组、信号投递原理的同学，评价会高于一个实现了所有功能但说不清 `setsid()` 为什么必要的同学。
...
**PTY 与进程管理**
1. 为什么 Pane 子进程在 `fork()` 后要调用 `setsid()`？不调用会怎样？
2. PTY master 和 slave 各自在哪个进程中打开和使用？为什么子进程不需要持有 master fd？
3. `fork()` 后子进程继承了父进程的所有 fd，哪些 fd 必须在子进程中关闭？为什么？
4. 子进程 `exec` shell 之后，shell 如何知道自己的终端大小？

**信号**
5. `Ctrl+C` 产生的 SIGINT 是如何到达 Pane 中的前台进程组的？信号经过了哪些环节？
6. 如果两个 Pane 的子进程属于同一个 Session，按 `Ctrl+C` 时会发生什么？
7. SIGWINCH 是谁发送的？Server 在什么时机需要主动向 Pane 发送 SIGWINCH？
8. 如何安全地处理 SIGCHLD？在信号 handler 中可以调用哪些函数？

**I/O 与通信**
9. `poll()` 和 `epoll` 在实现上的核心区别是什么？你选择了哪个，为什么？
10. Client 的终端为什么必须设为 raw mode？如果不设会有什么具体表现？
11. 描述一下一个按键从用户敲下键盘到最终出现在 Pane 程序中的完整数据路径
12. Server 和 Client 之间的协议你是怎么设计的？为什么这样设计？

**架构与设计决策**
13. 你的 Server 是单线程还是多线程？这个决策的依据是什么？
14. Detach 后再 Reattach，Client 如何获取当前的屏幕内容？
15. 如果一个 Pane 中的程序以每秒 10MB 的速度输出数据，你的 Server 会怎样处理？

### 5.3 评分标准

- **理解深度 > 功能完整度**：能解释清楚"为什么"比"实现了什么"更重要
- **坦诚分享失败**：描述你踩过的坑、AI 给出的错误建议、你如何发现并修正，这些都是加分项
- **AI 协作过程**：展示你如何与 AI 协作，特别是 AI 无法一次性解决的问题，你是如何通过拆解问题、构造最小复现、多轮迭代来攻克的

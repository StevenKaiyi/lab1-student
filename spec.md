...
实现一个简化版终端复用器 mini-tmux。mini-tmux 管理多个伪终端（Pseudo-terminal, PTY）窗格（Pane），允许用户在单个终端窗口中同时运行多个 shell 或程序，支持窗格的创建、销毁和焦点切换。
...
- **Server 进程**：后台常驻，管理所有 Pane 及其关联的 PTY。负责 fork 子进程、分配 PTY、转发 IO、投递信号。
- **Client 进程**：前台交互，连接到 Server，显示当前窗格布局并接收用户输入。
...
./mini-tmux attach       # 重新连接到已有 Server
```
...
在 Client 中输入以下命令（以冒号开头）操作 Pane：
...
只有获得焦点的 Pane 接收用户键盘输入。

## Detach 与 Reattach

- `Ctrl+B` 后按 `d`：Client detach（断开），Server 和所有 Pane 继续运行
- `./mini-tmux attach`：重新连接到 Server
...
2. **窗口大小**：Pane 创建时设置正确的 `winsize`（通过 `TIOCSWINSZ`），Client resize 时更新并发送 `SIGWINCH`
...
5. `:pipeout` 需要 Server 额外管理一条 pipe 和子进程，注意清理时机
6. Client 应该将 Pane 的 PTY 输出原样透传到终端，不要过滤或修改
